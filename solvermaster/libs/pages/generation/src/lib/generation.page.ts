...
@Component({
  ...
  template: `
    <div class="max-w-[820px] mx-auto">
      <sm-step-title eyebrow="Krok 03 · Generowanie rozwiązań" title="Generowanie">Macierz TRIZ i SCAMPER równolegle, plus własny koncept.</sm-step-title>
      ...
      <div class="flex items-center justify-between gap-3 mt-9 pt-6 border-t border-line">
        <a routerLink="/mapping" class="inline-flex items-center gap-2 border border-line bg-white hover:bg-slate-50 text-[14px] font-medium px-4 py-2.5 rounded-lg text-slate-600 transition"><span aria-hidden="true">←</span> Wstecz</a>
        <a routerLink="/evaluation" class="inline-flex items-center gap-2 bg-accent hover:bg-accent-ink text-white text-[14px] font-medium px-5 py-2.5 rounded-lg shadow-sm transition">Dalej <span aria-hidden="true">→</span></a>
      </div>
    </div>
  `,
})
