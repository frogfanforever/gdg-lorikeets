...
@Component({
  ...
  template: `
    <nav aria-label="Kroki" class="mb-12">
      <ol class="grid grid-cols-2 md:grid-cols-5 gap-x-4 gap-y-5">
            ...
            <div class="flex items-center gap-2.5">
              <span
                ...
                [class]="$index <= current
                  ...
                  : 'bg-white border border-line text-slate-400'">{{ $index + 1 }}</span>
              <div class="min-w-0">
                <div class="font-mono text-[10px] uppercase tracking-[0.14em] text-slate-400">Krok {{ ($index + 1).toString().padStart(2, '0') }}</div>
                <div class="text-[13px] font-medium truncate" [class]="$index === current ? 'text-ink' : 'text-slate-500'">{{ s.label }}</div>
              </div>
            </div>
